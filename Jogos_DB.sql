...
BEGIN
    ...
    BEGIN
        IF(@TIME_MANDANTE <> (SELECT NOME FROM Times WHERE NOME = @TIME_MANDANTE))
            INSERT INTO Times(NOME) VALUES(@TIME_MANDANTE)

        IF(@TIME_VISITANTE <> (SELECT NOME FROM Times WHERE NOME = @TIME_VISITANTE))
            INSERT INTO Times(NOME) VALUES(@TIME_MANDANTE)
...
INSERT INTO Jogos(TIME_MANDANTE, PLACAR_MANDANTE, TIME_VISITANTE, PLACAR_VISITANTE, COMPETICAO, Ano)
...
BEGIN
    ...
    BEGIN
        IF NOT EXISTS (
            SELECT 1 FROM TimeAno
            WHERE NOME = @TIME_MANDANTE
              AND COMPETICAO = @COMPETICAO
              AND ANO = @ANO
        )
        BEGIN
            INSERT INTO TimeAno (NOME, COMPETICAO, Ano)
